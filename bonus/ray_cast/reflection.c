...
/*   Updated: 2022/08/15 17:29:32 by dkim2            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								...
								t_vec3 hit_point)
{
	...
	if (ra < 0)
		...
		ra = 1;
	obj_color = get_texture_color(hit_obj, &(hit_obj->textures[T_TEXTURE]), hit_point);
...
static t_vec3	apply_normal_map(const t_quadrics *Q, t_vec3 hit_point, t_vec3 normal)
{
	t_color	color;
	t_vec3	bump_vec;
	t_vec3	temp_uv[2];
...
static t_color	apply_height_map(const t_quadrics *Q, t_vec3 hit_point, t_color color)
{
	t_color	gray;
						...
						t_vec3 view_point)
{
	...
	normal = apply_normal_map(hit_obj, hit_point, normal);
	...
	radiosity[0] = color_add(color_add(radiosity[0], radiosity[1]), radiosity[2]);
	return (apply_height_map(hit_obj, hit_point, radiosity[0]));
}
