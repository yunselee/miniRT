...
/*   Updated: 2022/08/18 14:57:18 by dkim2            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_vec3 hit_point)
{
	...
	obj_clr = get_texture_color(Q, &(Q->textures[T_TEXTURE]), hit_point);
	obj_clr = color_disruption(Q, hit_point, obj_clr);
	...
	while (light != NULL)
	{
		...
		if (diffuse > EPSILON)
		{
			clr_tmp.red = (int)((float)light->color.red / 255 * obj_clr.red);
			clr_tmp.green = (int)((float)light->color.green / 255 \
									* obj_clr.green);
			clr_tmp.blue = (int)((float)light->color.blue / 255 * obj_clr.blue);
			clr_tmp.alpha = 0;
