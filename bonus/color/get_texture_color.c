...
/*   Updated: 2022/08/14 01:24:10 by dkim2            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_color	get_image_pixel_color(const t_xpm *xpm, int x, int y, int width)
{
	int	int_color;

	int_color = (xpm->addr)[y * width + x];
	return (int_to_color((int)int_color));
}
...
static t_color	get_plane_texture_color(const t_quadrics *Q, const t_xpm *texture, t_vec3 point)
{
	...
	return (get_image_pixel_color(texture, pixel[0], pixel[1], texture->img_width));
}
