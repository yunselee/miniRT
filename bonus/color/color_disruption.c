...
/*   Updated: 2022/08/12 14:37:29 by dkim2            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_color	disrupt_plane_color(const t_quadrics *Q, t_vec3 point)
{
	...
	if (disrupt == 0)
		return (Q->color);
	return (rgb_color(255 - Q->color.red, \
						255 - Q->color.green, \
						255 - Q->color.blue));
}

t_color	color_disruption(const t_quadrics *Q, t_vec3 point)
{
	...
	if (Q->disruption == FALSE)
		return (Q->color);
	if (Q->type == Q_PLANE)
		return (disrupt_plane_color(Q, point));
